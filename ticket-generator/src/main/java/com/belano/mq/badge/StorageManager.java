...
/**
 * Responsible for writing data to a file in <b>JSON</b> and provides a file to other classes to read data.
 *
...
public class StorageManager {
  ...
  public synchronized void saveNewData(JSONObject dataToSave) throws DataDidNotSaveException {
    ...
    if (filename != null) {
      try {
        ...
        pw.write(jsonToString(dataToSave));
        pw.flush();
        pw.close();
      }
  ...
  private static String jsonToString(JSONObject dataToSave) {
    try {
      return dataToSave.toString(4);
    } catch (JSONException e) {
      throw new IllegalArgumentException("Could not convert data to string", e);
    }
  }
