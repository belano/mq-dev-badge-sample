...
import com.belano.mq.events.Event;
import com.belano.mq.events.Venue;
import com.belano.mq.exceptions.CouldNotDeductTicketsException;
import com.belano.mq.exceptions.DataDidNotSaveException;
import java.io.File;
...
/**
 * Deals with the creation and alterations of {@link Event}'s. The {@link EventManager} also deals with the reading of
 * data and the formatting of the data to be written.
 *
...
public class EventManager {
  ...
  private static final Logger logger = Logger.getLogger("com.belano.mq.badge");

  /**
   * {@link StorageManager} that may be used in the reading and writing of data.
   */
  private StorageManager storageManager = null;

  /**
   * The list of events that are created by this {@link EventManager}
   */
  ...
  /**
   * Creates an {@link EventManager} with a specified {@link StorageManager} for reading and writing data too.
   *
   * @param storageManager The {@link StorageManager} that will provide details such as filename and provide the ability
   *                       to save new data.
   */
  ...
  /**
   * Creates the base {@link EventManager} for future adaptations and alterations.
   */
  public EventManager() {
  }
  ...
  /**
   * Provides the {@link EventManager} with a {@link StorageManager}. Allows for the current {@link EventManager} to
   * setup its events using the file provided by the {@link StorageManager}.
   *
   * @param storageManager The {@link StorageManager} with a specified file that would be used in reading and writing.
   */
  ...
  /**
   * Sets up the events from the current {@link StorageManager} specified.
   *
   ...
   *
   * @param storageManager The {@link StorageManager} which it will read the data from
    ...
    if (!setup) {
      ...
      if (eventsArray != null) {
        for (int n = 0; n < eventsArray.length(); n++) {
          ...
          try {
            eventObject = eventsArray.getJSONObject(n);
            createEventFromJson(eventObject);
          } catch (JSONException e) {
            logger.warning(() -> "Could not get event from array");
            e.printStackTrace();
          }
        }
      } else {
  ...
  /**
   * Subtracts the tickets requested from a client and saves the data internally (and to a file if there is a correctly
   * setup {@link StorageManager} set for the current {@link EventManager}).
   *
   ...
   * @throws CouldNotDeductTicketsException Thrown when tickets quantity for an event could not be altered further
   */
  ...
  /**
   * Subtracts the tickets requested from a client and saves the data internally (and to a file if there is a correctly
   * setup {@link StorageManager} specified).
   *
   * @param eventID         ID of the event that is being requested.
   * @param numberRequested Number of tickets that is requested for said event.
   * @param storageManager  The {@link StorageManager} from which it will save data to.
   * @throws CouldNotDeductTicketsException Thrown when tickets quantity for an event could not be altered further
   */
  public void subtractTickets(Integer eventID, Integer numberRequested, StorageManager storageManager)
      throws CouldNotDeductTicketsException {
    ...
    if (ticketsLeft > 0) {
      ...
      logger.info(
          String.format("There are now %s for event: %s", bookableEvent.getCapacity(),
              bookableEvent.getTitle()));
    } else {
  ...
  /**
   * Creates a JSON object with details from the events that are stored in the current {@link EventManager}.
   *
   ...
   * @throws JSONException Thrown when an error occurred when creating JSON objects and JSON Arrays.
   */
  ...
  /**
   * Creates an event from a JSON object that contains all the details needed. Each event created from this is added to
   * the total event list for this current {@link EventManager}
   *
  ...
  private void createEventFromJson(JSONObject eventObject) {
    ...
    if (name != null && location != null && time != null &&
      ...
      logger.finer(
          "Info needed for a bookable event: EventID, Name, Location, Date, Time, TicketQuantity[Standard[Price, Quantity]] ([] = JSON array)");
    }
  }

  /**
   * Adds an event ID and bookable event to the current {@code HashMap} of events for the current instance of {@link
   * EventManager}.
   *
  ...
  /**
   * Adds a {@code HashMap} of events to the current {@code HashMap} of events for the current instance of {@code
   * EventManager}.
   *
   * @param bookableEvents {@code HashMap} of events to add to the {@code HashMap}.
   */
  ...
  /**
   * Gets a JSON array from a filename specified.
   *
  ...
  private JSONArray getJsonArray(String filename, String arrayToGet) {
    ...
    try {
      ...
      ja = jo.getJSONArray(arrayToGet);
    } catch (NoSuchElementException e) {
      logger.warning("There is not data in '" + filename + "'");
    } catch (JSONException e) {
      logger.warning("Data in filename '" + filename + "' could not be parsed as a JSON Array");
   ...
   * @param ticketQuantity A number of tickets available for the {@link Event}.
   * @return A {@link Event} containing all the information provided.
   */
   ...
   * @return Return a fully set Venue containing the details provided.
   */
   ...
   * @return A date that is gathered from the string. Can return null.
   */
   ...
   * @return A time that is gathered from the string. Can return null.
   */
