...
/**
 * Used to connect to topics and queues to listen for messages and create appropriate responses. Main use of this class
 * is to listen to responses put on a {@code purchase} queue and then send a response to a {@code confirmation} queue.
 *
...
public class Cashier implements Runnable {
  ...
  /**
   * Flag used to alert the cashier to stop listening and start closing connections.
   */
  ...
  /**
   * {@link EventManager} is responsible for dealing with ticket requests such as subtracting tickets and saving to a
   * file.
   */
  private EventManager eventManager = null;

  /**
   * {@link EnvSetter} is responsible for settin the correct configuration for the connection to MQ topics and queues.
   */
  private EnvSetter envSetter = null;

  /**
   * The Context is the current MQ context. This is set when the connection is created for the {@link Cashier}.
   */
  private JMSContext context = null;

  /**
   * A JMSProducer is what is required when wanting to send messages to a queue or topic.
   */
  private JMSProducer producer = null;

  /**
   * A JMSConsumer is what is required when receiving messages from a queue or topic.
   */
  ...
  /**
   * This constructor will provide the {@link Cashier} with all the details to make a successful pipeline of information
   * and allow tickets to be through the cashier. Without a {@link EventManager} no tickets would be deducted from
   * memory or saved. Without a {@link EnvSetter} no details would be provided for the MQ connections.
   *
   * @param eventManager The {@link EventManager} that will be dealing with ticket subtractions.
   * @param envSetter    The {@link EnvSetter} that will be providing MQ connection details.
   */
  ...
  /**
   * Creates the base {@link Cashier}. A {@link EventManager} will be needed if dealing with requests is the aim. A
   * {@link EnvSetter} will also need to be set or this instance as it will provide the {@link Cashier} with sufficient
   * details for creating MQ connections.
   */
  public Cashier() {
  }
  ...
  /**
   * Checks whether the cashier is ready and is listening for a response to be put on the queue.
   *
  ...
  /**
   * Provides the ability to start this class as a thread for background processing.
   */
  public void run() {
  ...
  /**
   * Stops the cashier from listening to anymore messages and moves onto closing the connections that are open for a
   * clean shutdown.
   */
  public void close() {
    cancel = true;
  ...
  /**
   * Connects to 2 queues that are specified. The {@code receiveFrom} queue is where the {@link Cashier} would get its
   * client's requests from. Following from that is the {@code sendTo} queue which is the destination of where a
  ...
  /**
   * Listens to a queue and waits for a message to be received. When a message is received then it will process the
   * message and create a response to be published to another queue. Destinations are specified in the creation of te
   * Cashier connections.
   */
  public void waitForRequest() {
    if (consumer != null && producer != null && context != null) {
      ...
      while (!cancel) {
        ...
        try {
          message = consumer
              .receive(10000); // Wait 10 seconds for a message. If no message is received, the method returns null.
        } catch (JMSRuntimeException e) {
  ...
  /**
   * Processes the message received and creates a response depending on the request and abilities of the application.
   *
  ...
  private void processMessage(Message message) {
    ...
    if (sendTo != null) {
      try {
        Objects.requireNonNull(responseMessage).setJMSCorrelationID(message.getJMSCorrelationID());
